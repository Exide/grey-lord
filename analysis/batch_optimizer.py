...
# Add tools directory to path to find calculate_context_window
...
def find_optimal_batch_size(available_memory_gb=10.0, target_seq_lens=None):
    """Find optimal batch sizes for different context window lengths"""
    ...
    print(f"\n{'Context':<8} | {'Batch=1':<10} | {'Batch=2':<10} | {'Batch=4':<10} | {'Batch=8':<10} | {'Max Batch':<10}")
    print("-" * 70)
    ...
    for rec in recommendations:
        ...
        print(f"Context {rec['seq_len']:<4}: Use --batch-size {rec['max_batch']:<2} ({rec['memory_usage']:.0f}MB, ~{speedup_text})")
    
    print(f"\n=== Key Insights ===")
    print(f"• Your GPU can handle much larger batch sizes!")
    print(f"• Larger batches = better GPU utilization = faster training")
    print(f"• Start with batch size 4-8 for most context window sizes")
    ...
    print(f"\n🎯 **Sweet Spot**: --context-window {best_rec['seq_len']} --batch-size {best_rec['max_batch']}")
